<script setup>
document.title = '轩铭2512 - 资源投稿 - 通知管理'
import { ref } from 'vue'
...
const url = ref('')
const emailresult = ref(false)
const webhookresult = ref(false)
async function getUserInfo() {
  ...
  if (noticesetting.includes('resourcecreator_email_result')) {
    emailresult.value = true
  }
  if (noticesetting.includes('resourcecreator_webhook_result')) {
    webhookresult.value = true
  }
}
getUserInfo()
function copy() {
...
async function changeEmailResult(zt) {
  let wz = '开启'
  if (zt == false) {
    wz = '关闭'
  }
...
async function changeWebhookResult(zt) {
  let wz = '开启'
  if (zt == false) {
    wz = '关闭'
  }
...
<template>
  <div>
    <div class="cz">
      ...
      <tiny-alert :closable="false"
        description="设置后，服务器会向地址发送 HTTP POST 请求推送通知，请求体为 JSON 对象，含有 data、timeStamp 和 signature 三个键。data 键值为 string 类型，反序列化后的 JSON 对象含有 noticeName 键和其他键，其他键由通知类型决定；timeStamp 键值为 number 类型，用于请求防重放、验签；signature 键值为 string 类型，用于验签。目前提供两种方式验证请求是否来源于轩铭 2512 服务器：下载下方证书使用其中的公钥验签（具体步骤可参考下方文档）、验证请求 IP 是否为 81.68.129.229，推荐使用前者。"></tiny-alert>
      <div class="sp">
      ...
      <div class="sp">
        ...
        <div class="item">
          <tiny-switch v-model="webhookresult" @change="changeWebhookResult"></tiny-switch>
        </div>
      </div>
    </div>
    <tiny-dialog-box class="dialog" :visible="webhookdialog" title="设置 Webhook 推送地址" @close="closeWebhookDialog">
      <div class="dialog-cz">
        ...
        <div>提示：</div>
        <div>1. 让 Webhook 推送地址/xm2512webhooktoken/resourcecreator 返回 webhookToken。</div>
        <div>2. 点击“设置”按钮向服务器发送设置请求。</div>
        <div>3. 服务器收到设置请求后会向输入的 Webhook 推送地址/xm2512webhooktoken/resourcecreator 发送 HTTP GET 请求，检查响应体是否是正确的 webhookToken。
        </div>
